...
static wndresult_t PrintLawyers(const struct List *dbcontents){
    //создаём новый список ссылок на структуры, в котором сохраним ссылки на те из них, что содержат уникальные имена адвокатов
    struct List *lawyers = calloc(1, sizeof(struct List));
    if (lawyers==NULL) return ERR;
    for (struct Node *n=dbcontents->head;n!=NULL;n=n->next){
        bool isUnique = TRUE;
        for (struct Node *l=lawyers->head;l!=NULL;l=l->next){
            int cmpl = strncmp(l->data->lawyer, n->data->lawyer, strlen(n->data->lawyer));
            if (cmpl==0){
                isUnique = FALSE;               //если результирующий список уже содержит это имя - пропуск записи
                break;
            }
        }
        if (isUnique) ListAppend(n->data,lawyers);
    }
...
static wndresult_t SearchName(struct Vertex *treeroot){
    ...
    if (contents==NULL) return ERR;
    TreeSearch(dpsitr, lawyer, treeroot, contents); //ищем в дереве
