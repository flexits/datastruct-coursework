...
void TreeSearch(const char* name, const char* lawyer, struct Vertex *root, struct List *result){
    ...
    for (;current!=NULL;){
        int cmp = strncmp(name, current->data->name, strlen(name)); //0 для пустых строк
        if (cmp>0){                                     //начав с корня, движемся в левое или правое поддерево...
            current = current->right;
        } else if (cmp<0){
            current = current->left;
        } else {                                        //...до нахождения совпадения, но не останавливаемся, а рекурсивно продожаем поиск в найденном поддереве слева направо -
                                                        //т.к. частичное совпадение не обязательно единственное
            TreeSearch(name, lawyer, current->left, result);
            int cmpl = strncmp(lawyer, current->data->lawyer, strlen(lawyer));
            if (cmpl==0) ListAppend(current->data, result); //дополнительная проверка: добавляем указатель на запись в список результатов только при совпадении поля lawyer
