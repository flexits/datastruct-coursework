 ...
 */
struct Vertex* TreeInsert(struct DbRecord *data, struct Vertex *root);      //добавляет запись из БД в АВЛ-дерево, возвращает вершину дерева
void TreeDestruct(struct Vertex *root);                                     //удаляет все вершины дерева, высвобождая занятую память
void TreeSearch(const char* key, struct Vertex *root, struct List *result); //поиск записей в дереве, для которых поле name начинается с key, и добавление ссылок на найденное в список
